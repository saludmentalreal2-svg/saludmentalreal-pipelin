import os, sys, json, pickle, random, asyncio, requests, subprocess, base64
...
def crear_video_largo(audio_file, videos_horizontal, output_file):
    ...
    for i in range(clips_necesarios):
        ...
        subprocess.run([
            ...
            '-t', str(dur_por_clip), '-c:v', 'libx264', '-pix_fmt', 'yuv420p',
            '-r', '25', '-an', clip
        ], capture_output=True)
        clips.append(clip)
...
def crear_short(audio_file, videos_vertical, output_file):
    ...
    subprocess.run([
        ...
        '-t', str(duracion + 2), '-c:v', 'libx264', '-pix_fmt', 'yuv420p',
        '-r', '25', '-an', video_mudo
    ], capture_output=True)
    subprocess.run([
        'ffmpeg', '-y', '-i', video_mudo, '-i', audio_file,
...
def main():
    ...
    crear_short(audio_short, videos_v, video_short)
